...
def ai_trading():
    ...
    def fetch_usd_ohlcv(timeframe, limit):
        try:
            ...
            logger.error(f"Error fetching USD OHLCV data: {e}")
            return pd.DataFrame()
    ...
    def prepare_ml_data():
        ...
        if trades_df.empty:
            logger.warning("거래 내역이 없습니다. 머신러닝 모델을 실행할 수 없습니다.")
...
공포와 탐욕 지수: {json.dumps(fear_greed_index)}"""
    ...
    try:
        ...
        openai_target_btc_ratio = current_btc_ratio
    ...
    final_target_btc_ratio = (openai_target_btc_ratio + current_btc_ratio) / 2 if ml_decision == "hold" else openai_target_btc_ratio
    ...
    if difference > 0:
        ...
        if buy_amount_krw > 5000:
            logger.info(f"### Buy Order Executed: {buy_amount_krw:.2f} KRW worth of BTC ###")
        ...
        if sell_amount_btc * current_btc_price > 5000:
            ...
            logger.info("### Sell Order Failed: Insufficient BTC amount ###")
    else:
